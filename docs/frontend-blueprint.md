...
- 인증 완료 후 포트폴리오 보유 여부 확인
- 초기 유저는 직접 포트폴리오 추가 화면을 먼저 거친 뒤 메인 탭 진입
...
- 초기 포트폴리오는 전용 직접 추가 화면, 거래 추가는 사후 보정용이라는 안내
- 종목 선택
- 매수/매도 전환
- 수량, 가격, 가격 범위
- 코멘트와 공개 범위
...
1. 직접 포트폴리오 추가 흐름에서 평균 매수시점/보유 비중 입력 확장
2. 댓글, 리액션, 팔로우, 알림 설정 등 write 액션 범위 확장
3. 포트폴리오 상세, 거래 수정/삭제, 사용자 검색 화면 추가
4. KIS OAuth 시작 이후 callback exchange와 계좌 연결 저장소 구현
